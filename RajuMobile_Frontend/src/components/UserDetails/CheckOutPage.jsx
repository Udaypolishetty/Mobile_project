              ...
              <AnimatedSection direction="left" delay={160}>
                <div className="co-card">
                  ...
                  {PAYMENT_METHODS.map((m) => (
                    <div key={m.id} className={`pay-option ${payment === m.id ? "selected" : ""}`} onClick={() => setPayment(m.id)}>
                      <div className={`radio-dot ${payment === m.id ? "on" : ""}`} />
                      <span style={{ fontSize: "20px" }}>{m.icon}</span>
                      <div>
                        <p style={{ fontSize: "13px", fontWeight: 700, color: "#1e293b" }}>{m.label}</p>
                        <p style={{ fontSize: "11px", color: "#94a3b8" }}>{m.sub}</p>
                      </div>
                    </div>
                  ))}
                </div>
