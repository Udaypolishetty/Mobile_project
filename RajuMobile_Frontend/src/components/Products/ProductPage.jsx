...
//       ? `http://127.0.0.1:8000${product.images[selectedImage].image}`
//       : product.image
//   }
//   alt={product.name}
// />
...
/* ─── ImageZoom — fixed: product ref removed, proper img fallback ── */
function ImageZoom({ src, alt, badge }) {
  const [zoomed, setZoomed] = useState(false);
  const [pos, setPos]       = useState({ x: 50, y: 50 });
  const ref = useRef(null);
...
useEffect(() => {
  ...
  // Build images array — support both {id, image} objects AND plain strings
