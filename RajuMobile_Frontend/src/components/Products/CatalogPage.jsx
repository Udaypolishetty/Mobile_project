...
//                 {categories.map((cat) => (
//                   <li key={cat} onClick={() => setCategory(cat)} className={`cursor-pointer text-sm px-3 py-2 rounded-lg transition ${selectedCat === cat ? "bg-cyan-500 text-white font-semibold" : "text-gray-600 hover:bg-gray-50"}`}>
//                     {cat}
//                   </li>
//                 ))}
//               </ul>
...
//               <input type="range" min={500} max={50000} step={500} value={maxPrice} onChange={(e) => setMaxPrice(Number(e.target.value))} className="w-full accent-cyan-500" />
...
//                 <input type="checkbox" checked={saleOnly} onChange={(e) => setSaleOnly(e.target.checked)} className="accent-cyan-500" />
//                 Sale Items Only
//               </label>
//             </div>
//           </aside> */}
...
//                 {filtered.map((p, i) => (
//                   <AnimatedSection key={p.id} direction="up" delay={Math.min(i * 50, 400)}>
